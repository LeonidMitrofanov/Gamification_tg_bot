...
def get_message(key: str, locale: str, messages: dict[str, dict[str, str]]) -> str:
    ...
    if (locale is None) or not (locale in Language.ALL):
        logger.warning(f"Locale '{locale}' not found for message key '{key}', "
                       f"falling back to default locale '{Language.DEFAULT}'.")
    ...
    try:
        ...
        for message_elem in root.findall('message'):
            ...
            for text_elem in message_elem.findall('text'):
                ...
                # Join all child elements and text to preserve the XML structure
                text = ''.join(ET.tostring(e, encoding='unicode') for e in text_elem)
                if text == "":
                    text = ''.join(text_elem.itertext()).strip()  # TODO: fix
                if lang and text:
                    locales[lang] = text
                else:
